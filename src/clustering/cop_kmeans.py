"""
   This scipt implements "Constrained Kmeans with Background Knowledge".
   Code reference: https://github.com/Behrouz-Babaki/COP-Kmeans
"""
...
def cop_kmeans(dataset, k, ml=[], cl=[], initialization='kmpp',
               max_iter=300, tol=1e-4, centers=None):
...
def tolerance(tol, dataset):
    n = len(dataset)
    dim = len(dataset[0])
    averages = [sum(dataset[i][d]
                    for i in range(n))/float(n) for d in range(dim)]
